...
% %% Calcualte human links - adult
% % units [mm]
% filelist = dir('data_fortraining/human_P01*.xlsx');
% sz_filelist = size(filelist,1);
% adult_l1_all = zeros(sz_filelist,1);
% adult_l2_all = zeros(sz_filelist,1);
% for i_f = 1:sz_filelist
%     filedir = filelist(i_f).folder;
%     filename_temp = filelist(i_f).name;
%     temp_full = strcat(filedir,'\',filename_temp);
%     [temp_adult_l1, temp_adult_l2] = cal_human_links(temp_full);
%     adult_l1_all(i_f,1) = temp_adult_l1;
%     adult_l2_all(i_f,1) = temp_adult_l2;
% end
% adult_l1 = mean(adult_l1_all);
% adult_l2 = mean(adult_l2_all);
% 
% %% Calcualte human links - kid
% % units [mm]
% filelist = dir('data_fortraining/human_P02*.xlsx');
% sz_filelist = size(filelist,1);
% adult_l1_all = zeros(sz_filelist,1);
% adult_l2_all = zeros(sz_filelist,1);
% for i_f = 1:sz_filelist
...
%     adult_l2_all(i_f,1) = temp_adult_l2;
% end
% kid_l1 = mean(adult_l1_all);
% kid_l2 = mean(adult_l2_all);
...
suff_ratio = size(low_suff,2)/sz_total
