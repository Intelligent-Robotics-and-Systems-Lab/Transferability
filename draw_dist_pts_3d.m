...
function draw_dist_pts_3d(ave_final_dist_struct1, x_pos_dist, y_pos_dist, z_pos_dist,...
...
%             i_y_v = indx_zeroy;
%             % find <0
%             indx_newz = find(z_pos_dist<1e-6);
%             sz_newz = size(indx_newz,2);
...
%                 % for 3d illustration 
%                 % z>0, y>0, z-x+1>0
                if temp_tr_y > 0 && temp_tr_z-temp_tr_x>0
%                 if temp_tr_y > 0 && temp_tr_z>0
                    % nothing
                    a = 1; 
                else
...
%                     temp_color = [155/255 155/255 155/255];
                    ...
                    if target_mi >= 1e-6
%                         if temp_distance > 0.98
                            scatter3(temp_tr_x,temp_tr_y,temp_tr_z,70,'filled',...
                            'MarkerEdgeColor','k',...
...
%                 % z>0, y>0, abs(z-x)> 0.01
%                 if temp_tr_y > 0 && abs(temp_tr_z-temp_tr_x) < 0.01
%                     % draw 
%                     % set color based on manipulability measure
%                     % 0 to 10 
%                     % R 255 ~ 0
...
% 
%                     if target_mi >= 1e-6
% %                         scatter3(temp_tr_x,temp_tr_y,temp_tr_z,50,'MarkerEdgeColor',temp_color,...
% %                       'MarkerFaceColor',temp_color,'LineWidth',0.5);
%                         
%                         scatter3(temp_tr_x,temp_tr_y,temp_tr_z,180,'s','filled',...
...
%                 
%                 % z>0, y>0, abs(z-x)> 0.01
%                 if abs(temp_tr_y) < 1e-6 && temp_tr_z-temp_tr_x > 0 
%                     % draw 
%                     % set color based on manipulability measure
%                     % 0 to 10 
%                     % R 255 ~ 0
